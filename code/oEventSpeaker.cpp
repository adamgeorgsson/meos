...
void oEvent::updateComputerTime(bool considerDate) {
  ...
  if (!useLongTimes() && !considerDate)
    computerTime = (((24+2+st.tm_hour)*timeConstHour+st.tm_min*timeConstMinute+st.tm_sec*timeConstSecond - ZeroTime)%(24*timeConstHour)-2*timeConstHour) * (1000/timeConstSecond) + 0 /* TODO: std::tm has no milliseconds */;
  else {
    std::tm stDate = {};
    ...
    // std::tm has no milliseconds field; sub-second offset dropped

    int64_t zero = SystemTimeToInt64TenthSecond(stDate);
    int64_t now = SystemTimeToInt64TenthSecond(st);
    computerTime = (now - zero) * (1000 / timeConstSecond) + 0 /* TODO: std::tm has no milliseconds */ % (1000 / timeConstSecond);
  }
}
