...
const wstring &oEvent::formatRogainingStringAux(const oPrintPost &pp, const oListParam &par, const RogainingLegInfo *rgLeg) const {
  ...
  if (pp.type != lString && (wsptr == 0 || wsptr->empty()) && bfw[0] == 0)
    ...
    if (pp.text.empty())
      ...
      swprintf(bfw, pp.text.c_str(), wsptr->c_str());
  ...
  switch (lt) {
    ...
    case unused_EStdTeamResultListLeg: {
      ...
      if (li.lp.getLegNumberCoded() != 1000)
        swprintf(title, (lang.tl(L"Resultat efter sträcka X#" + li.lp.getLegName())+L" - %%s").c_str());
      else
        swprintf(title, (lang.tl(L"Resultat efter sträckan")+L" - %%s").c_str());
    ...
    case unused_EStdTeamResultListLegLARGE: {
      wchar_t title[256];
      if (li.lp.getLegNumberCoded() != 1000)
        swprintf(title, (L"%%s - "+lang.tl(L"sträcka X#" + li.lp.getLegName())).c_str());
      else
        swprintf(title, (L"%%s - "+lang.tl(L"slutsträckan")).c_str());
