...
// Kept for compatibility with meos.cpp call-site; std::mutex needs no explicit init/cleanup.
void initMySQLCriticalSection(bool /*init*/) {
}

bool isThreadReconnecting()
{
  std::lock_guard<std::mutex> lg(CS_MySQL);
...
void MySQLReconnect::process(gdioutput &gdi, oEvent *oe, AutoSyncType ast)
{
  ...
  if (mysqlStatus==1) {
    ...
    mysqlConnecting = 1;
    std::thread(reconnectThread, oe).detach();
    hThread = nullptr;
    interval = 1;
  }
}
...
void MySQLReconnect::status(gdioutput &gdi) {
  ...
  if (interval>0){
    ...
    gdi.addTimer(gdi.getCY(),  gdi.getCX()+10, timerCanBeNegative, int(getTimeoutElapsedMs()/1000));
  }
  else {
