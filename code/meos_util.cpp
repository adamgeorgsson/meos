...
int getRelativeDay() {
  ...
  uint64_t ft_sec = (uint64_t)((int64_t)t + 11644473600LL);
  uint64_t days = ft_sec / (uint64_t)(24 * timeConstSecPerHour);
  days -= 400 * 365;
  return int(days);
}
...
__int64 SystemTimeToInt64TenthSecond(const std::tm &st) {
  std::tm tmp = st;
  time_t t = meos_timegm(&tmp);  // treat tm fields as UTC (matches original behavior)
  __int64 ft100ns = ((__int64)t + 11644473600LL) * 10000000LL;
  return ft100ns / (__int64)(1000 * 1000 * 10 / timeUnitsPerSecond);
}

std::tm Int64TenthSecondToSystemTime(__int64 time) {
  std::tm st = {};
  __int64 ft100ns = time * (__int64)(1000 * 1000 * 10 / timeUnitsPerSecond);
  time_t t = (time_t)(ft100ns / 10000000LL) - 11644473600LL;
  meos_gmtime(&t, &st);
  return st;
}
//2014-11-03 07:02:00
...
wstring addOrSubtractDays(const wstring& m, int days) {
  ...
  time_t t = meos_timegm(&st);  // treat date fields as UTC
  t += (time_t)days * 24 * timeConstSecPerHour;
  std::tm new_st = {};
  meos_gmtime(&t, &new_st);
