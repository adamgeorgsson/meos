...
// File-local helpers shared across oListInfo.cpp / oListInfoGen.cpp split files.
// Include AFTER all other includes (relies on StdAfx.h, oEvent.h, oListInfo.h,
// gdioutput.h, localizer.h, meos_util.h).
...
static inline std::pair<std::wstring, bool> getControlName(const oEvent &oe, int courseContolId) {
  ...
  return std::make_pair(toS, name);
}

static inline std::wstring getFullControlName(const oEvent &oe, int ctrl) {
...
static inline void getResultTitle(const oEvent &oe, const oListParam &lp, std::wstring &title) {
