...
- **`extern` declarations of global variables not copied to new files**: When splitting, `extern` declarations of global variables (e.g., `extern int defaultCodePage;`) in the original `.cpp` file are not automatically copied to the new file. After splitting, grep the new file for any identifiers that were declared `extern` in the original file and add the missing `extern` declarations.
