...
// checksum functions (defined in metalist.cpp in legacy — local copies here)
...
static int checksum(const wstring &str) {
  int ret = 0;
  for (size_t k = 0; k < str.length(); k++)
    ret = ret * 19 + str[k];
  return ret;
...
// oEvent::getGeneralResult stub — returns base GeneralResult (no dynamic module registry yet)
shared_ptr<GeneralResult> oEvent::getGeneralResult(const string &/*tag*/, wstring &/*srcDMY*/) {
  return make_shared<GeneralResult>();
}
...
void GeneralResult::calculateTeamResults(vector<oTeam *> &teams, 
  ...
  try {
    ...
    prepareCalculations(*teams[0]->getEvent(), classResult, clsId, runners, teams, inputNumber);

    vector<GRSortInfo> teamScore(teams.size());
    for (size_t k = 0; k < teams.size(); k++) {
      if (resType == oListInfo::Classwise) {
        pClass tc = teams[k]->getClassRef(true);
        teamScore[k].principalSort = tc ? tc->getSortIndex() * 50000 + tc->getId() : 0;
      }
      else
...
void GeneralResult::calculateIndividualResults(vector<oRunner *> &runners,
  ...
  try {
    ...
    prepareCalculations(*runners[0]->getEvent(), classResult, clsId, runners, noTeams, inputNumber);
    //bool classSort = resType == oListInfo::Global ? false : true;
    vector<GRSortInfo> runnerScore(runners.size());
    for (size_t k = 0; k < runners.size(); k++) {
      const oRunner *r = runners[k];
      if (resType == oListInfo::Classwise) {
        { pClass rc = r->getClassRef(true);
          runnerScore[k].principalSort = rc ? rc->getSortIndex() * 50000 + rc->getId() : 0; }
      }
...
void DynamicResult::save(xmlparser &xml) const {
  ...
  if (origin.empty())
    origin = L"(" + getLocalDate() + L")";  // No gEvent ref in cross-platform build
...
// checksum: see static definitions at top of this file
...
// debugDumpVariables removed — GUI-only (was: void DynamicResult::debugDumpVariables(gdioutput &, bool))
